...
/**
 * Shared helpers for PTB (product tile block) grids fed by
 * main-category product APIs.
 */
...
export function getPtbAssetUriFromApiPath(pathOrUrl: string): string {
  return getAssetUriFromApiPath(pathOrUrl);
}

/** Normalize category names for fuzzy matching. */
...
export function pickPtbVariantPricing(p: unknown): {
  ...
  const v = variants.find((x: any) => x && x.inStock === true) ?? variants[0];
  ...
  const vidNum = typeof rawVid === "string" ? Number.parseInt(rawVid, 10) : Number(rawVid);
  const variantId = Number.isFinite(vidNum) && vidNum > 0 ? Math.floor(vidNum) : undefined;
